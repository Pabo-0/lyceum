...
def build_neo4j_verification_cypher() -> str:
    statements = [
        ...
        "MATCH (:Chunk)-[r:MENTIONS]->(:Concept) RETURN count(r) AS chunk_mentions",
        "MATCH (:Concept)-[r:RELATED_TO]->(:Concept) RETURN count(r) AS related_to_candidates",
        "MATCH (:Concept)-[r:PREREQUISITE_CANDIDATE]->(:Concept) RETURN count(r) AS prerequisite_candidates",
    ]
    return _join_statements(statements)
